...
#define NUM_BALLS         6               // Number of bouncing balls
...
void loop() {
  for (int i = 0; i < NUM_BALLS; i++) {
    ...
    // A little kinematics equation calculates position as a function of time, acceleration (gravity) and initial velocity
    h[i] = 0.5 * GRAVITY * pow(tCycle[i] / 1000.0, 2.0) + vImpact[i] * tCycle[i] / 1000.0;

    if (h[i] < 0) {
      h[i] = 0;                            // If the ball crossed the threshold of the "ground," put it back on the ground
      vImpact[i] = COR[i] * vImpact[i];    // and recalculate its new upward velocity as its old velocity * COR
      tLast[i] = millis();
